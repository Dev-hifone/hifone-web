      ...
      <div className="flex gap-8">
        ...
        <div className="w-56 shrink-0 space-y-3">
          <div className="bg-gradient-to-br from-[#E31E24] to-[#C01017] rounded-2xl p-4 text-center">
            ...
            <p className="text-white/80 text-xs mb-3">Get an upfront price before we start</p>
            <Link to="/book" onClick={onClose}
...
export const Navbar = () => {
  ...
  // Mobile repair items built from brands
  const mobileRepairItems = brands.map(b => ({
    label: b.brand,
    href: b.routeKey ? `/devices/${b.routeKey}` : `/devices?category=${b.category}`,
  }));
  ...
  return (
    <>
      <div className="fixed top-0 left-0 right-0 z-50">
          ...
          <nav className="max-w-7xl mx-auto px-4 lg:px-8">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="hidden lg:flex items-center gap-0.5">
                ...
                {/* Repair */}
                <div onMouseEnter={() => handleMouseEnter('repair')} onMouseLeave={handleMouseLeave}>
                  <button className={cn(
                    ...
                    Repair
                    <ChevronDown className={cn('w-3.5 h-3.5 transition-transform duration-200', activeMenu === 'repair' && 'rotate-180')} />
                  </button>
                </div>
                ...
                <div onMouseEnter={() => handleMouseEnter('services')} onMouseLeave={handleMouseLeave}>
                  <button className={cn(
                    ...
                    Services
                    <ChevronDown className={cn('w-3.5 h-3.5 transition-transform duration-200', activeMenu === 'services' && 'rotate-180')} />
                  </button>
                </div>
        ...
        {/* Mega menus — OUTSIDE header, full width dropdown */}
        <AnimatePresence>
          {activeMenu === 'repair' && (
            <motion.div
              ...
              <div className="py-3 max-h-[75vh] overflow-y-auto">
                ...
                <button onClick={() => setOpenMobile(openMobile === 'repair' ? null : 'repair')}
                  ...
                  <span className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-[#E31E24]" /> Repair
                  </span>
                ...
                <button onClick={() => setOpenMobile(openMobile === 'services' ? null : 'services')}
                  className="flex items-center justify-between w-full px-4 py-3 text-base font-medium text-white hover:bg-white/10 rounded-xl mx-2 transition-colors">
                  <span className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-[#E31E24]" /> Services
                  </span>
